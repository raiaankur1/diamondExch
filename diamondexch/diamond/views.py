...
import random
from .backends import PhoneUsernameAuthenticationBackend as PhAB
...
def forgotpwd1(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messagehandler = MessageHandler(
                verify_number, otp).send_otp_via_message()
            red = redirect('forgotpwd2', uid=verifyobj.uid)
            red.set_cookie("can_otp_enter", True, max_age=300)
            return red
        except get_user_model().DoesNotExist:
            messages.error(
                request, f"No account is registered with this phone number {phone_number} !"
            )
            return redirect('signup1')
    return render(request, "diamond/forgotPassword1.html", {})


def forgotpwd2(request, uid):
    if request.user.is_authenticated:
        return redirect('home')
    verifyobj = Verifyotp.objects.get(uid=uid)
    if request.method == "POST":
        ...
        if request.COOKIES.get('can_otp_enter') != None:
            ...
            if (verifyobj.otp == otp_code):
                ...
                return redirect('forgotpwd3', uid=verifyobj.uid)
            messages.error(
                request, "OTP doesn't match!"
            )
            return redirect('forgotpwd2', uid=verifyobj.uid)
        messages.error(
            request, "OTP expired! You took longer than 5 minutes!"
        )
        return redirect("forgotpwd1")
    # return render(request,"otp.html",{'id':uid})
    phone_number = verifyobj.phone_number
    content = {
        "phone_number": phone_number,
    }
    return render(request, "diamond/forgotPassword2.html", content)


def forgotpwd3(request, uid):
    # form = UserCreationForm()
    if request.user.is_authenticated:
        return redirect('home')
    verifyobj = Verifyotp.objects.get(uid=uid)
    ...
    if request.method == "POST":
        try:
            update_user = get_user_model().objects.get(phone_number=phone_number)
            password = request.POST.get('password1')
            Cpassword = request.POST.get('password2')
            ...
            messages.success(
                request, "Password updated for " + f"{phone_number}"
            )
            return redirect("login")
        except get_user_model().DoesNotExist:
    ...
    return render(request, "diamond/forgotPassword3.html", content)
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                print(user.password == cd['password'])
                get_user_model().objects.get(phone_number=cd['phone_number'])
                user = PhAB.authenticate(
                ...
                if user is not None:
                    ...
                    # print('not logged in')
                    messages.error(
                        request, "Password entered doesn't match!", 'danger')
                    return redirect('login')
            except get_user_model().DoesNotExist:
                messages.error(
                    request, f"No account is registered with this phone number {cd['phone_number']}!"
                )
                return redirect('signup1')
